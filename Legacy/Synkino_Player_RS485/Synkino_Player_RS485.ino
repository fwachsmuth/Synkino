 ...
 *  HM-10 flashen: https://forum.arduino.cc/index.php?topic=334466.15, 
...
void processMessage () {
  if (message.address == RCPT_AUDIO 
   || message.address == RCPT_EYE_AUDIO 
   || message.address == RCPT_UI_AUDIO
   || message.address == RCPT_ALL ) {   // could just check for bit 2 here, too
    Serial.print("Adr: ");
    ...
    switch(message.command) {
      case ADJUST_SPEED:
        //DELETE ppmOffset = atol(parameterFromRemote);
        adjustRate(message.parameter);
        break;
      case PLAY_TRACK:
        musicPlayer.pausePlaying(false);
        break;
      case PREP_TRACK:
        startPlay(message.asciis);
      ...
      case STOP_TRACK:
        ...
        break;
      case NAME_TRACK:
        findFilenameByNumber(message.asciis);
        break;
      case ADJUST_VOLUME:
      default:
...
void startPlay(char trackName[]) {
//  Serial.print("Übergebener Trackname kurz: ");
//  Serial.println(trackName);
  if (! musicPlayer.startPlayingFile(trackName)) {
    sendMessage(RCPT_UI, SHOW_TEXT , 666, "Could not open Track");
    Serial.print("Could not open ");
    Serial.println(trackName);
  } else {
    //sendMessage(RCPT_EYE_UI, SHOW_TEXT, 888, "Ready for Playback, awaiting projector start");
...
void findFilenameByNumber(char trackPrefix[]) {
  ...
  while (n < nMax && file.openNext(&dirFile, O_READ)) {
    if (!file.isSubDir() && !file.isHidden()) {   // Skip directories and hidden files
      ...
      if (strncmp(name, trackPrefix, 3) == 0) {
//      Serial.println(name); 
        sendMessage(RCPT_UI, FOUND_NAME , -1, name);
      } 
    }
    file.close();
  }
