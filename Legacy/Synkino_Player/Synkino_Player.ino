 ...
 * 
 *  TODO
 *  √ Volume wieder auf 20/20 stellen
 *  √ Umstellen auf SDfat
 *  √ Neben DSP auch Sampling Rate Adjustments einbauen
 *  √ Synchronisation mit externem Takt hinbekommen
 *  √ Umlaufblende erkennen
 *  √ Rundungsfehler beim casten auf long fixen
 *  √ ppm clampen
 *  √ Testfilm drehen
 *  √ SD Card listen bzw. Files finden http://forum.arduino.cc/index.php?topic=173562.0
 *  
 *  Umstellen von RS232 auf RS485: http://www.gammon.com.au/forum/?id=11428
 *  Bluetooth Modul einbauen
 *  Anlernen neuer Soundtracks
 *  Sampling und Pitching mergen
 *  Plugin im EEPROM ablegen und von dort laden
 *  RAM sparen: Serial.println(F("Started playing"));
 *  HM-10 flashen: https://forum.arduino.cc/index.php?topic=334466.15, http://forum.arduino.cc/index.php?topic=393655
 *  
 */

 
// patches240
// Allow sample rate finetuning 
...
// Define pins used for the breakout board
...
//int pitch = 16384;
// int potiPin = 0;
// signed long potiValue;
unsigned long playBeginMillis;

byte modePrintedFlag = 0;
...
//int integerFromController = 0;
//float floatFromController = 0.0;

char searchpath[numChars] = "/";
...
void setup() {
  ...
  while (! Serial);
  Serial.println(F("Synkino Debug Console"));
  ...
  Serial.println(F("SD OK! Listing:"));
  ...
  musicPlayer.sciWrite(VS1053_REG_WRAM, 0x0080);
  Serial.println(F("15/16 resampling on: PLAYMODE_RESAMPLER_TEST"));
...
void executeCommandFromRemote() {
  if (strcmp(commandFromRemote,"ADJ") == 0) {
    ppmOffset = atol(parameterFromRemote);
    adjustRate(ppmOffset);

  } else if (strcmp(commandFromRemote,"PLAY") == 0) {
    startPlay();
  
  } else if (strcmp(commandFromRemote,"PREP") == 0) {
    startPlay();
    musicPlayer.pausePlaying(true);
    
  } else if (strcmp(commandFromRemote,"PAUSE") == 0) {
    if (strcmp(parameterFromRemote,"ON") == 0) {
      musicPlayer.pausePlaying(true);
    } else if (strcmp(parameterFromRemote,"OFF") == 0) { 
      musicPlayer.pausePlaying(false);
    }
 
  } else if (strcmp(commandFromRemote,"STOP") == 0) {
//    stop();
 
  } else if (strcmp(commandFromRemote,"NAME") == 0) {
    findFilenameByNumber();
    //searchpath[] = "/" + *parameterFromRemote + "*.*";
    //SD.ls(searchpath, 0);
  }
  newData = false;
}
...
void startPlay() {
  if (! musicPlayer.startPlayingFile(parameterFromRemote)) {
    sendWithStartEndMarkers("FAIL",parameterFromRemote);
    Serial.print(F("Could not open "));
    Serial.println(parameterFromRemote);
  } else {
    sendWithStartEndMarkers("READY",parameterFromRemote);
    Serial.print(F("Now ready for playback: "));
    Serial.println(parameterFromRemote);
  }
}
...
void checkDebugKeys() {
  if (Serial.available()) {
    ...
    if (c == 't') {
      ...
      Serial.print(millis() - playBeginMillis);
    ...
    if (c == 's') {
    //        pitch = -16384;
    //        musicPlayer.sciWrite(0xC, pitch);
    ...
    if (c == 'x') {
      // Disable 15-16 Resampler
      musicPlayer.sciWrite(VS1053_REG_WRAMADDR, 0x1e09);
      musicPlayer.sciWrite(VS1053_REG_WRAM, 0x0000);
      Serial.println(F("15/16 resampling off"));
    }
    if (c == 'r') {
      // Enable 15-16 Resampler
      musicPlayer.sciWrite(VS1053_REG_WRAMADDR, 0x1e09);
    ...
    if (c == 'R') {
      // Enable 15-16 Resampler
      musicPlayer.sciWrite(VS1053_REG_WRAMADDR, 0x1e09);
      musicPlayer.sciWrite(VS1053_REG_WRAM, 0x0040);
      Serial.println(F("15/16 resampling on: PLAYMODE_RESAMPLER_ON"));
    }
  }    
}

void adjustRate(signed long ppm2) {
  musicPlayer.sciWrite(VS1053_REG_WRAMADDR, 0x1e07);
  musicPlayer.sciWrite(VS1053_REG_WRAM, ppm2);
  musicPlayer.sciWrite(VS1053_REG_WRAM, ppm2 >> 16);
  /* oldClock4KHz = 0 forces adjustment calculation when rate checked. */
  musicPlayer.sciWrite(VS1053_REG_WRAMADDR, 0x5b1c);
  musicPlayer.sciWrite(VS1053_REG_WRAM, 0);
  /* Write to AUDATA or CLOCKF checks rate and recalculates adjustment. */
  musicPlayer.sciWrite(VS1053_REG_AUDATA, musicPlayer.sciRead(VS1053_REG_AUDATA));
}

void usePoti() {
//  potiValue = analogRead(potiPin);
//  adjustRate(-100000 + 174 * potiValue);
//  Serial.print(potiValue);
//  Serial.print(" - ");
//  Serial.print(-100000 + 174 * potiValue);
//  Serial.println(" ");
//  delay(50);
}
...
//      Serial.print(n++);
//      Serial.print(" - ");
      file.getName(name, 131);
      //file.printName(&Serial);
      //Serial.print(name);
      // Serial.println();

      if (strncmp(name, parameterFromRemote, 2) == 0) {
        Serial.println(name);
      } 
    }
    file.close();
