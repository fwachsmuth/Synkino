 ...
 * - support m4a loading 
 * - volume erhöhen
 * - Änderungen an der Library diffen und dokumentieren
 * - Interrupt oder Timer-basiert arbeiten?
 * - Line out "glätten"
 * - ppm constrains > 40000? -> 15/16 resampler nach playbackstart enablen
 * - Schnelleren Resonator/Quarz besorgen 
 * - Resonator messen
 * 
 * 
 */
...
int initialPpmCorrection = 350; // This is the initial ppm correction value to
                                // make sure that 44,100 samples are played per AVR-second.
                                // This needs to be accurate to calculate the "playhead"
                                // position based on the sample count register.
                                // This value should adjust itself over time.
                                // 380 corrects ~32 Hz up. 1 should equal 2ppm.
int newPpmCorrection = 0;                              
...
void setup() {
  ...
  /* This is code taken over from the Adafruit lib approach. Adjust!

  musicPlayer.setVolume(15,15);
  musicPlayer.applyPatch(plugin, PLUGIN_SIZE);
  
  // This enables DSP stretching (needs pitchshifter plugin!)
  musicPlayer.sciWrite(0xC, 16384);
  ...
  But you must stick to 8.1 filenames, only 8 characters long, and 3 for the extension */
    ...
    while(currentmillis <= 10000000) {
      ...
      if (currentmillis % 1000 == 0) {
        ...
        Serial.print(F(" Samples / Sec. New ppm correction: "));
        ...
        Serial.println(newPpmCorrection);
